...
public:
    ...
    void SettingsTab(ImGuiStyle& style);
...
void CGui::ApplyTheme()
{
    ...
    case THEME_SKEET:
        ...
        style.Colors[ImGuiCol_Text] = ImAdd::HexToColorVec4(0xE0FFE0, 1.0f); // Light green text
        style.Colors[ImGuiCol_CheckMark] = style.Colors[ImGuiCol_Text];
        style.Colors[ImGuiCol_TextDisabled] = ImAdd::HexToColorVec4(0xA2A2A2, 1.0f);

        // Changed blue slider/active to bright green
        style.Colors[ImGuiCol_SliderGrab] = ImAdd::HexToColorVec4(0x4AFF4A, 1.0f); // Bright green
        style.Colors[ImGuiCol_SliderGrabActive] = ImAdd::HexToColorVec4(0x3CE03C, 1.0f); // Slightly darker green
        ...
        style.Colors[ImGuiCol_TextSelectedBg] = style.Colors[ImGuiCol_SliderGrab]; // Now green
    ...
    case THEME_RED:
        ...
        break;
    case THEME_WHITE:
        style.Colors[ImGuiCol_WindowBg] = ImAdd::HexToColorVec4(0x1F1F1F, 1.0f); // Unchanged (dark)
        style.Colors[ImGuiCol_PopupBg] = ImAdd::HexToColorVec4(0x272727, 1.0f);  // Unchanged (dark)
        style.Colors[ImGuiCol_ChildBg] = ImAdd::HexToColorVec4(0x2F2F2F, 1.0f);  // Unchanged (dark)

        // Changed from green to white:
        style.Colors[ImGuiCol_Text] = ImAdd::HexToColorVec4(0xFFFFFF, 1.0f); // White (was light green)
        style.Colors[ImGuiCol_CheckMark] = ImAdd::HexToColorVec4(0xFFFFFF, 1.0f); // White (inherits text)
        style.Colors[ImGuiCol_TextDisabled] = ImAdd::HexToColorVec4(0xA2A2A2, 1.0f); // Unchanged (gray)

        // Sliders changed from green to white:
        style.Colors[ImGuiCol_SliderGrab] = ImAdd::HexToColorVec4(0xFFFFFF, 1.0f); // White (was bright green)
        style.Colors[ImGuiCol_SliderGrabActive] = ImAdd::HexToColorVec4(0xDDDDDD, 1.0f); // Light gray (active state)

        style.Colors[ImGuiCol_ScrollbarGrab] = ImAdd::HexToColorVec4(0x2F2F2F, 1.0f); // Unchanged (dark)
        style.Colors[ImGuiCol_ScrollbarGrabHovered] = ImAdd::HexToColorVec4(0x2F2F2F, 1.0f); // Unchanged
        style.Colors[ImGuiCol_ScrollbarGrabActive] = ImAdd::HexToColorVec4(0x2F2F2F, 1.0f); // Unchanged

        style.Colors[ImGuiCol_ScrollbarBg] = ImVec4(0, 0, 0, 0); // Unchanged (transparent)

        // Selection highlight changed to white (translucent):
        style.Colors[ImGuiCol_TextSelectedBg] = ImAdd::HexToColorVec4(0xFFFFFF, 0.3f); // Was green slider color

        style.Colors[ImGuiCol_Border] = ImAdd::HexToColorVec4(0x272727, 1.0f); // Unchanged (dark)
        style.Colors[ImGuiCol_Separator] = style.Colors[ImGuiCol_Border]; // Unchanged

        // All button/frame/header colors remain unchanged (dark):
        style.Colors[ImGuiCol_Button] = ImAdd::HexToColorVec4(0x1F1F1F, 1.0f);
        style.Colors[ImGuiCol_ButtonHovered] = ImAdd::HexToColorVec4(0x1F1F1F, 0.7f);
        style.Colors[ImGuiCol_ButtonActive] = ImAdd::HexToColorVec4(0x1F1F1F, 0.5f);
...
//std::string GenerateRandomCommitHash() {
...

    {
        ...
        // Content
        {
            ImGui::BeginChild("SideBar", ImVec2(SideBarWidth, 0), ImGuiChildFlags_Border, ImGuiWindowFlags_NoBackground);
            {
                ...
                ImGui::PopStyleColor();
                {
                    ImGui::SetCursorPos({ ImGui::GetWindowWidth() / 2 - ImGui::CalcTextSize(std::string("Mocha " + std::string((GIT_COMMIT_HASH))).c_str()).x / 2, style.WindowPadding.y });
                    ImGui::BeginGroup();
                    {
                        ImGui::Text("Priv Base");
                        ImGui::SameLine(ImGui::CalcTextSize("Priv Base").x);
                        ImGui::SetCursorPosY(ImGui::GetCursorPosY() + 4);
                        ImGui::TextDisabled(std::string(" " + std::string((GIT_COMMIT_HASH))).c_str());
                    }
                    ImGui::EndGroup();
                }
                ImGui::EndChild();
                ...
                ImGui::PushStyleColor(ImGuiCol_FrameBg, style.Colors[ImGuiCol_PopupBg]);
                {
                    ...
                    ImAdd::SeparatorText("SETTINGS");
                    {
                        ...
                        ImAdd::RadioFrameIcon(GetTabName(MenuPage_Settings), GetTabIconTexture(MenuPage_Settings, iSelectedPage == MenuPage_Settings), &iSelectedPage, MenuPage_Settings, radioSize);
                    }
